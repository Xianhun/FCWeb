...
// 弹框
function message({icon,content,timeout=2000,closable=false,transition=300}) {
    $modal({
        ...
        timeout: timeout, // 单位 ms  显示多少毫秒后关闭弹框 （ confirm 下无效 | 不传默认为 2000ms | 最短显示时间为500ms）
        content: content, // 提示文字
        center: false,// 是否绝对居中 默认为false  设置true后   top无效
        top: 100, //距离顶部距离 单位px
        transition: transition, //过渡动画 默认 200   单位ms
        closable: closable, // 是否显示可关闭按钮  默认为 false
    })
}

function dialog({icon,title,content = '',timeout=2000,mask=true,transition=300,maskClose=false}){
    $modal({
        ...
        mask:mask, // 是否显示遮罩层   默认为 false
        pageScroll: true, // 是否禁止页面滚动
        width:300, // 单位 px 默认显示宽度 最下默认为300
        maskClose: maskClose // 是否点击遮罩层可以关闭提示框 默认为false
    })
}

function confirm({icon,title,content,transition=300,cancelText="取消",confirmText="确认",cancel=(close)=>{close()},confirm=(close)=>{close()}}){
    $modal({
        ...
        cancelText: cancelText,// 取消按钮 默认为 取消
        confirmText: confirmText,// 确认按钮 默认未 确认
        cancel:cancel,
        confirm:confirm
    })
}